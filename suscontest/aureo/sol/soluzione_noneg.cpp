// @check-accepted: esempi N_100,V_i>=0 V_i>=0
// @check-wrong-answer: N_100 full
#include <vector>
#include <iostream>
using namespace std;
...
long long sortedSum(vector<long long> a)
{
    ...
    {
        t += prev;
        t += tree.query(tree.st, 0, ST_SIZE - 1, (long long)a[i] + 1, ST_SIZE - 1);

        t += (tree.query_smaller(tree.st, 0, ST_SIZE - 1, 0, (long long)a[i]) + 1ll) * (long long)a[i];

        prev = t;
        t = abs(t) % MOD;
        ans *= t;
        ans %= MOD;
        tree.update(tree.st, 0, ST_SIZE - 1, (long long)a[i]);
        t = 0;
    }
    return ans;
}
